...
# This module provides functions to calculate various volatility indicators used in stock
# market analysis. These include Bollinger Bands, ATR, historical volatility, standard
# deviation, Keltner Channels, Chaikin Volatility, Donchian Channels, and Price %B.
# """
...
def calculate_keltner_channels(
    ...
    if len(closes) < window or len(highs) < window or len(lows) < window:
        raise ValueError("Not enough data for Keltner Channels.")
    closes_series = pd.Series(closes[-window:])
    ema = float(closes_series.ewm(span=window).mean().iloc[-1])
    atr = calculate_atr(highs, lows, closes, window)
    return {
        "ema": round(ema, 4),
        "upper_channel": round(ema + factor * atr, 4),
...
def calculate_donchian_channels(
    ...
    if len(highs) < window or len(lows) < window:
        raise ValueError("Not enough data for Donchian Channels.")
    return {
        "upper_channel": round(float(max(highs[-window:])), 4),
        "lower_channel": round(float(min(lows[-window:])), 4),
    }
...
def calculate_price_percent_b(prices: list[float], window: int = 20, num_std: int = 2) -> float:
    ...
    return round((series.iloc[-1] - lower) / (upper - lower), 4)
...
def analyze_volatility(symbol: str, data: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        result: dict[str, Any] = {
            ...
            "chaikin_volatility": calculate_chaikin_volatility(highs, lows),
            "donchian_channels": calculate_donchian_channels(highs, lows),
            "percent_b": calculate_price_percent_b(prices),
        }
